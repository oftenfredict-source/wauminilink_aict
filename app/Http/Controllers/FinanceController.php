...
class FinanceController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // Calculate pending and committed amounts for accurate utilization display
        $budgetIds = $currentBudgets->pluck('id');
        ...
        $currentBudgets->each(function ($budget) use ($pendingExpensesByBudget) {
            $budget->pending_expenses_amount = (float) ($pendingExpensesByBudget[$budget->id] ?? 0);
            ...
            $budget->utilization_committed_percentage = $budget->total_budget > 0
                ? round(($budget->total_committed / $budget->total_budget) * 100, 2)
                : 0;

            // Update flags to use committed amounts
            $budget->is_over_budget = $budget->total_committed > $budget->total_budget;
            $budget->is_near_limit = $budget->utilization_committed_percentage >= 90 && $budget->utilization_committed_percentage < 100;
        });
    ...
    public function budgets(Request $request)
    {
        ...
        // Add pending and committed data to each budget
    ...
    /**
     * Re-synchronize all budgets spent_amount from expenses table
     */
    public function reSyncBudgets()
    {
        $user = auth()->user();
        if (!$user->isTreasurer() && !$user->isAdmin()) {
        ...
        try {
            ...
            foreach ($budgets as $budget) {
                $actualSpent = Expense::where('budget_id', $budget->id)
                    ->where('status', 'paid')
                    ->where('approval_status', 'approved')
                    ->sum('amount');

                if ((float) $budget->spent_amount !== (float) $actualSpent) {
                    $budget->update(['spent_amount' => $actualSpent]);
                    $updatedCount++;
                }
            }

            DB::commit();
            return redirect()->back()->with('success', "Budgets re-synchronized successfully. {$updatedCount} budgets updated.");
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error('Budget re-sync failed: ' . $e->getMessage());
            return redirect()->back()->with('error', 'Budget re-synchronization failed: ' . $e->getMessage());
        }
    }
    ...
    public function markExpensePaid(Expense $expense)
    {
        ...
        try {
            ...
            if ($expense->budget_id) {
                $budget = Budget::find($expense->budget_id);
                if ($budget) {
                    // spent_amount is now updated automatically by Expense model events
