...
<div class="container-fluid px-4">
    <div class="d-flex flex-wrap align-items-center justify-content-between mt-4 mb-3 gap-2">
        <h2 class="mb-0">Edit Promise Guest</h2>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('promise-guests.update', $promiseGuest) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" 
                               id="name" name="name" value="{{ old('name', $promiseGuest->name) }}" required>
                        @error('name')
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            @php
                                ...
                                $phoneDisplay = old('phone_number', $promiseGuest->phone_number);
                                if (strpos($phoneDisplay, '+255') === 0) {
                                    $phoneDisplay = substr($phoneDisplay, 4);
                                }
                            @endphp
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="email" class="form-control @error('email') is-invalid @enderror" 
                               id="email" name="email" value="{{ old('email', $promiseGuest->email) }}">
                        @error('email')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control @error('promised_service_date') is-invalid @enderror" 
                               ...
                               value="{{ old('promised_service_date', $promiseGuest->promised_service_date->format('Y-m-d')) }}" required>
                ...
                <div class="row mb-3">
                        ...
                        <select class="form-select @error('service_id') is-invalid @enderror" 
                            ...
                            @foreach($upcomingServices as $service)
                                <option value="{{ $service->id }}" 
                                        ...
                                        {{ old('service_id', $promiseGuest->service_id) == $service->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" 
                            ...
                            <option value="pending" {{ old('status', $promiseGuest->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="notified" {{ old('status', $promiseGuest->status) == 'notified' ? 'selected' : '' }}>Notified</option>
                            <option value="attended" {{ old('status', $promiseGuest->status) == 'attended' ? 'selected' : '' }}>Attended</option>
                            <option value="cancelled" {{ old('status', $promiseGuest->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        ...
                        <textarea class="form-control @error('notes') is-invalid @enderror" 
                                  id="notes" name="notes" rows="3">{{ old('notes', $promiseGuest->notes) }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save me-2"></i>Update Promise Guest
                    </button>
