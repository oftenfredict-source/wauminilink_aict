            ...
            <tbody>
                @foreach($records as $record)
                    <tr data-record-type="expense" data-record-id="{{ $record->id }}">
                        ...
                        <td>{{ $record->expense_date ? \Carbon\Carbon::parse($record->expense_date)->format('M d, Y') : '-' }}
                        </td>
                        ...
                        <td>
                            @if(isset($record->is_fundable))
                                @if($record->is_fundable)
                                    <span class="badge bg-success" title="Budget has sufficient allocated funds">
                                        <i class="fas fa-check-circle me-1"></i>Funded
                                    </span>
                                @else
                                    <span class="badge bg-warning text-dark"
                                        title="Insufficient allocated funds (Available: TZS {{ number_format($record->available_fund_amount ?? 0, 0) }})">
                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                @if(isset($canApprove) && $canApprove)
                                    <button class="btn btn-success btn-sm" onclick="approveRecord('expense', {{ $record->id }})"
                                        title="Approve">
                                        <i class="fas fa-check"></i>
                                    </button>
                                    <button class="btn btn-danger btn-sm" onclick="rejectRecord('expense', {{ $record->id }})"
                                        title="Reject">
                                        <i class="fas fa-times"></i>
                                    </button>
                                @endif
                                <button class="btn btn-info btn-sm" onclick="viewDetails('expense', {{ $record->id }})"
                                    title="View Details">
