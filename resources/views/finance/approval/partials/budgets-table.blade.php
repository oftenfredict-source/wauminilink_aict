            ...
            <tbody>
                @foreach($records as $record)
                    <tr data-record-type="budget" data-record-id="{{ $record->id }}">
                        ...
                        <td>
                            <span class="badge bg-primary">{{ ucfirst($record->budget_type) }}</span>
                        </td>
                        <td>
                        ...
                        <td>
                            {{ $record->start_date ? \Carbon\Carbon::parse($record->start_date)->format('M d') : '-' }} -
                            {{ $record->end_date ? \Carbon\Carbon::parse($record->end_date)->format('M d, Y') : '-' }}
                        </td>
                        <td>
                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                @if(isset($canApprove) && $canApprove)
                                    <button class="btn btn-success btn-sm" onclick="approveRecord('budget', {{ $record->id }})"
                                        title="Approve">
                                        <i class="fas fa-check"></i>
                                    </button>
                                    <button class="btn btn-danger btn-sm" onclick="rejectRecord('budget', {{ $record->id }})"
                                        title="Reject">
                                        <i class="fas fa-times"></i>
                                    </button>
                                @endif
                                <button class="btn btn-info btn-sm" onclick="viewDetails('budget', {{ $record->id }})"
                                    title="View Details">
