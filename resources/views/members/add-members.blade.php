                            ...
                            <div class="col-md-4">
                                <div class="form-floating">
                                    <input type="text" class="form-control check-envelope" name="envelope_number"
                                        id="envelope_number" value="{{ old('envelope_number') }}" required maxlength="3"
                                        minlength="3" pattern="\d{3}" title="Please enter exactly 3 digits">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-floating">
                                            <input type="text" class="form-control check-envelope"
                                                name="spouse_envelope_number" id="spouse_envelope_number" maxlength="3"
                                                minlength="3" pattern="\d{3}" title="Please enter exactly 3 digits">
                ...
                function addChild() {
                    ...
                    row.innerHTML = `
                                                                                                                                                                                                ...
                                                                                                                                                                                                <div class="col-md-2" id="childEnvelopeWrapper_${idx}" style="display:none;">
                                                                                                                                                                                                    ...
                                                                                                                                                                                                    <input type="text" class="form-control child-envelope check-envelope" name="children[${idx}][envelope_number]" maxlength="3" minlength="3" pattern="\d{3}" title="Please enter exactly 3 digits">
                    ...
                    if (step === 1) {
                        ...
                        reqNames.forEach(n => {
                            ...
                            if (el && (el.offsetParent !== null)) {
                                // Use checkValidity() to enforce pattern/minlength (especially for 3-digit envelope)
                                ok = check(el, el.checkValidity()) && ok;
                            }
                        });
                    ...
                    if (step === 4) {
                        ...
                        if (document.getElementById('marital_status').value === 'married') {
                            ...
                            if (sChurch.value === 'yes') {
                                // Use checkValidity() to enforce 3-digit length
                                ok = check(sEnv, sEnv.checkValidity()) && ok;
                            }
                        }
                        ...
                        if (childRows.length > 0) {
                            childRows.forEach(row => {
                                ...
                                if (envelope && envelope.offsetParent !== null && envelope.hasAttribute('required')) {
                                    // Use checkValidity() to enforce 3-digit length
                                    ok = check(envelope, envelope.checkValidity()) && ok;
                                }
                            });
                        }
...
@section('scripts')
    ...
    <script>
        ...
        // Add blur listener to block leaving field with invalid envelope
        document.querySelectorAll('.check-envelope').forEach(input => {
            input.addEventListener('blur', function () {
                if (this.value && !this.checkValidity()) {
                    const statusDiv = document.getElementById(this.id + '_status') || this.parentElement.querySelector('.envelope-status-msg');
                    if (statusDiv) {
                        statusDiv.innerHTML = '<i class="fas fa-exclamation-circle me-1"></i> Must be exactly 3 digits';
                        statusDiv.className = 'envelope-status-msg text-danger small mt-1 fw-bold bounce-animation';
                    }
                    this.classList.add('is-invalid');
                    // Optional: Scroll back and focus if user moves to another field with invalid data
                    // But maybe only if they actually try to click "Next" is better.
                    // The validateStep handles the blocking for "Next".
                }
            });
        });
