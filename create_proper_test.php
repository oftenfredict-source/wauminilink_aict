...
echo "Creating proper test data...\n";
...
for ($i = 4; $i >= 1; $i--) {
    ...
    $services[] = $service;
    echo "Created service ID {$service->id}: {$service->service_date} - {$service->theme}\n";
}
...
foreach ($members as $index => $member) {
    echo "Processing member: {$member->full_name} (ID: {$member->id})\n";
    ...
    if ($index % 2 == 0) {
        ...
        foreach ($services as $service) {
            ...
            echo "    - Attended service ID {$service->id}\n";
        }
...
echo "\nProper test data created successfully!\n";
...
echo "3. Test member history: http://127.0.0.1:8000/attendance/member/64/history\n";
